...
class FormatXTCEpixQuad(FormatXTC):
    ...
    def understand(image_file):
        try:
            ...
            return False
        return any("epixquad" in src.lower() for src in params.detector_address)

    def get_raw_data(self, index=None):
        if index is None:
            index = 0
        d = FormatXTCEpixQuad.get_detector(self, index)
    ...
    def get_detector(self, index=None):
        return FormatXTCEpixQuad._detector(self, index)
...
if __name__ == "__main__":
    for arg in sys.argv[1:]:
        ...
        print(FormatXTCEpixQuad.understand(arg))
