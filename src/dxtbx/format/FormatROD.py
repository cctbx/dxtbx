...
__copyright__= "Copyright 2018 United Kingdom Research and Innovation & 2022 Takanori Nakane"
...
class FormatROD(Format):
  ...
  def understand(image_file):
    try:
      hd = FormatROD._read_ascii_header(image_file)
    except (IOError, ValueError) as err:
      print(err)
      return False

    bhd = FormatROD._read_binary_header(image_file)
    return True
  ...
  @staticmethod
  def _read_binary_header(image_file, offset=256, nbytes=5120,
    general_nbytes=512, special_nbytes=768, km4gonio_nbytes=1024,
    statistics_nbytes=512, history_nbytes=2048):
    ...
    with FormatROD.open_file(image_file, 'rb') as f:
      ...
      f.seek(offset + 22)
      chip_npx_x, chip_npx_y, im_npx_x, im_npx_y = struct.unpack('<hhhh', f.read(8))
      f.seek(offset + 36)
      ...
      overflow_flag, overflow_after_remeasure_flag = struct.unpack('<hh', f.read(4))
      ...
      angles_in_deg = struct.unpack('<dddd', f.read(32)) # alpha, beta, gamma, delta
      f.seek(offset + general_nbytes + special_nbytes + 712)
      distance_mm = struct.unpack('<d', f.read(8))[0]

    return {'bin_x':bin_x, 'bin_y':bin_y, 'chip_npx_x':chip_npx_x,
            'chip_npx_y':im_npx_y, 'im_npx_x':im_npx_x, 'im_npx_y':im_npx_y,
            'gain':gain, 'overflow_flag':overflow_flag,
            'overflow_after_remeasure_flag':overflow_after_remeasure_flag,
            'overflow_threshold':overflow_threshold,
            'exposure_time_sec':exposure_time_sec,
            'overflow_time_sec':overflow_time_sec, 'detector_type':detector_type,
            'real_px_size_x':real_px_size_x, 'real_px_size_y':real_px_size_y,
            'start_angles_steps':start_angles_steps,
            'end_angles_steps':end_angles_steps, 'step_to_rad':step_to_rad,
            'beam_rotn_around_e2':beam_rotn_around_e2,
            'beam_rotn_around_e3':beam_rotn_around_e3,
            'alpha1_wavelength':alpha1_wavelength, 'detector_rotns':detector_rotns,
            'origin_px_x':origin_px_x, 'origin_px_y':origin_px_y,
  ...
  def _detector(self):
    gonio_angles = np.array(self._bin_header['start_angles_steps']) * np.array(self._bin_header['step_to_rad'])
    detector_rotns_rad = np.array(self._bin_header['detector_rotns']) / 180 * np.pi
    rot_e1 = np.array([ np.cos(detector_rotns_rad[0]), np.sin(detector_rotns_rad[0]), 0,
                       -np.sin(detector_rotns_rad[0]), np.cos(detector_rotns_rad[0]), 0,
                        0, 0, 1
                      ]).reshape(3, 3)
    rot_e2 = np.array([ 1, 0, 0,
                        0, np.cos(detector_rotns_rad[1]), np.sin(detector_rotns_rad[1]),
                        0, -np.sin(detector_rotns_rad[1]), np.cos(detector_rotns_rad[1])
                      ]).reshape(3, 3)
    rot_theta = np.array([ np.cos(gonio_angles[1]), 0, np.sin(gonio_angles[1]),
                           0, 1, 0,
                          -np.sin(gonio_angles[1]), 0, np.cos(gonio_angles[1])
                         ]).reshape(3, 3)
    ...
    origin_at_zero = np.array([-self._bin_header['origin_px_x'] * pixel_size_x,
    ...
    detector = self._detector_factory.make_detector(
      ...
      (0, self._bin_header['overflow_threshold'])) # not sure about min
  ...
  def _scan(self):
    ...
    for axis in [0, 3]: # 0 - OMEGA, 3 - PHI: FIXME: is it always PHI scan?
      start_angle = self._bin_header['start_angles_steps'][axis] * self._bin_header['step_to_rad'][axis] / np.pi * 180
      end_angle = self._bin_header['end_angles_steps'][axis] * self._bin_header['step_to_rad'][axis] / np.pi * 180
  ...
  def decode_TY6_oneline(self, linedata, w):
    ...
    for i in range(nrest):
      ...
      if (px < SHORT_OVERFLOW):
        ...
        ret[opos] = ret[opos - 1] + linedata[ipos:(ipos+4)].view(np.int32)[0]
        ipos += 4
      else:
        ret[opos] = ret[opos - 1] + linedata[ipos:(ipos+2)].view(np.int16)[0]
        ipos += 2
      opos += 1
