    ...
    def __call__(self, subset):
        ...
        for item in subset:
            try:
                ...
                beam_center, detector_address, distance, img, pixel_size, saturated_value, size, wavelength, active_areas = self.read(
                    item
                )
...
def run(argv=None):
    ...
    if len(paths) == 1:
        ...
        if (
            ...
            and command_line.options.num_images_max < len(imageset)
        ):
        ...
        if (
            command_line.options.num_images_max is not None
            and command_line.options.num_images_max < len(paths)
        ):
