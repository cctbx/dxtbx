    ...
    for item in iterable:
        ...
        for panel in detector:
            ...
            p_extent_two_theta = (
                ...
                * math.asin(
                    beam.get_wavelength()
                    / (2 * panel.get_max_resolution_at_corners(beam.get_s0()))
                )
                * 180
                / math.pi
            )
        ...
        if params.median_filter_size is not None:
            ...
            results = flex.double(
                median_filter(results.as_numpy_array(), size=params.median_filter_size)
            )

        # calculate standard devations
        std_devs = [
            math.sqrt((sums_sq[i] - sums[i] * results[i]) / counts[i])
            if counts[i] > 0
            else 0
