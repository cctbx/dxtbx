...
def to_imageset(input_filename, extra_filename=None):
    ...
    imageset = ImageSetFactory.from_template(
        template, image_range=image_range, check_format=False
    )[0]
...
class to_xds(object):
    ...
    def __init__(self, sweep):
        ...
        R = align_reference_frame(
            self.get_detector()[0].get_fast_axis(),
            (1, 0, 0),
            self.get_detector()[0].get_slow_axis(),
            (0, 1, 0),
        )
        ...
        self.detector_x_axis = (
            R * matrix.col(self.get_detector()[0].get_fast_axis())
        ).elems
        self.detector_y_axis = (
            R * matrix.col(self.get_detector()[0].get_slow_axis())
        ).elems
        ...
        self.rotation_axis = (
            R * matrix.col(self.get_goniometer().get_rotation_axis())
        ).elems
        return
    ...
    def XDS_INP(
        ...
        print(
            "NAME_TEMPLATE_OF_DATA_FRAMES= %s" % self.get_template().replace("#", "?"),
            file=out,
        )
