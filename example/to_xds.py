...
class to_xds:
    ...
    def XDS(self):
        ...
        beam = (
            R
            * self.get_xbeam().get_direction_c()
        ...
        print("STARTING_ANGLE= %.3f" % self.get_xscan().get_oscillation()[0])
        print("OSCILLATION_RANGE= %.3f" % self.get_xscan().get_oscillation()[1])
        print("X-RAY_WAVELENGTH= %.5f" % self.get_xbeam().get_wavelength())
