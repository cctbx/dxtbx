...
class XSweep2XDS:
    ...
    def XDS(self):
        ...
        beam = (
            R
            * self._xsweep.get_xbeam().get_direction_c()
        ...
        print("STARTING_ANGLE= %.3f" % self._xsweep.get_xscan().get_oscillation()[0])
        print("OSCILLATION_RANGE= %.3f" % self._xsweep.get_xscan().get_oscillation()[1])
        print("X-RAY_WAVELENGTH= %.5f" % self._xsweep.get_xbeam().get_wavelength())
...
if __name__ == "__main__":
    ...
    xs = XSweep2Factory.FromImages("noddy", XWavelength(), sys.argv[1:])
