...
class SingleFileReader(ReaderBase):
    ...
    def get_image_paths(self, indices=None):
        ...
        filenames = [
            self._format.get_image_file(i) for i in range(self._format.get_num_images())
        ]
...
class MemImageSet(ImageSet):
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, slice):
            ...
            return tuple(
                [img.get_raw_data(i) for i in xrange(len(self.get_detector()))]
            )
...
class ImageSweep(ImageSet):
    ...
    def __init__(
        self, reader, indices=None, beam=None, goniometer=None, detector=None, scan=None
    ):
        """Create the sweep.
...
class ImageSetFactory(object):
    ...
    def make_sweep(
        ...
        # Create the sweep object
        sweep = ImageSweep(
            reader, beam=beam, detector=detector, goniometer=goniometer, scan=scan
        )
