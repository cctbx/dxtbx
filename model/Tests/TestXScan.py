...
def work_helper_image_files():
    ...
    directory = os.path.join(os.environ["XIA2_ROOT"], "dxtbx", "model", "Tests")

    template = "image_###.dat"

    assert (
        len(XScanHelperImageFiles.template_directory_to_indices(template, directory))
        == 20
    )
    ...
    assert XScanHelperImageFiles.template_index_to_image(template, 1) == "image_001.dat"
    ...
    assert XScanHelperImageFiles.image_to_template("image_001.dat") == "image_###.dat"
...
def work_helper_image_formats():
    ...
    assert XScanHelperImageFormats.check_format(XScanHelperImageFormats.FORMAT_CBF)
