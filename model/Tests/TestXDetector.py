...
def TestXDetector():
    ...
    d = XDetectorFactory.Simple(
        100.0, (45.0, 52.0), "+x", "-y", (0.172, 0.172), (516, 590), (0, 1024), []
    )
    t = XDetectorFactory.TwoTheta(
        60.0,
        (35.0, 34.0),
        "+x",
        "+y",
...
def WorkXDetectorHelpers():
    compute_frame_rotation(
        (matrix.col((1, 0, 0)), matrix.col((0, 1, 0)), matrix.col((0, 0, 1))),
        (matrix.col((1, 0, 0)), matrix.col((0, 1, 0)), matrix.col((0, 0, 1))),
    )

    compute_frame_rotation(
        (matrix.col((-1, 0, 0)), matrix.col((0, 0, 1)), matrix.col((0, 1, 0))),
        (matrix.col((1, 0, 0)), matrix.col((0, 1, 0)), matrix.col((0, 0, 1))),
    )
