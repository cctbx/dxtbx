...
class Experiment(object):
    ...
    def __contains__(self, item):
        ...
        return (
            item is self.imageset
            or item is self.beam
            or item is self.detector
            or item is self.goniometer
            or item is self.scan
            or item is self.crystal
            or item is self.profile
        )
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, Experiment):
            return False
        return (
            self.imageset is other.imageset
            and self.beam is other.beam
            and self.detector is other.detector
            and self.goniometer is other.goniometer
            and self.scan is other.scan
            and self.crystal is other.crystal
            and self.profile is other.profile
        )
...
class ExperimentList(object):
    ...
    def replace(self, a, b):
        ...
        for i in self.indices(a):
            exp = self._data[i]
            if exp.imageset is a:
                exp.imageset = b
            elif exp.beam is a:
                exp.beam = b
            elif exp.detector is a:
                exp.detector = b
            elif exp.goniometer is a:
                exp.goniometer = b
            elif exp.scan is a:
                exp.scan = b
            elif exp.crystal is a:
                exp.crystal = b
            elif exp.profile is a:
                exp.profile = b
            else:
                raise ValueError("unidentified model %s" % a)
    ...
    def where(
        ...
        if [crystal, imageset, detector, beam, goniometer, scan, profile].count(
            None
        ) == 7:
            return []
        expt_indices = []
...
class ExperimentListFactory(object):
    ...
    def from_json(text, check_format=True):
        ...
        return ExperimentListFactory.from_dict(
            json.loads(text, object_hook=_decode_dict), check_format
        )
