...
def set_mosflm_beam_centre(detector, beam, mosflm_beam_centre, detector_distance=None):
...
def set_mosflm_beam_centre_old(detector, beam, mosflm_beam_centre):
    ...
    assert (
        matrix.col(new_beam_centre) - matrix.col(tuple(reversed(mosflm_beam_centre)))
    ).length() < 1e-4
