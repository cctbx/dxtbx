    ...
    def set_parent_frame(self, fast_axis, slow_axis, origin):
        ...
        for child in self:
            child.set_parent_frame(
                self.get_fast_axis(), self.get_slow_axis(), self.get_origin()
            )
...
class HierarchicalDetector(Detector):
    ...
    def is_similar_to(self, rhs):
        ...
        if not isinstance(rhs, HierarchicalDetector):
            return False
        return self._root.is_similar_to(rhs._root) and super(
            HierarchicalDetector, self
        ).is_similar_to(rhs)
    ...
    def __eq__(self, rhs):
        ...
        if not isinstance(rhs, HierarchicalDetector):
            return False
        return self._root == rhs._root and super(HierarchicalDetector, self).__eq__(rhs)
