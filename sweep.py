...
class sweep_of_images(object):
    ...
    def __init__(
        ...
        self._image_number_range = (min(self._image_numbers), max(self._image_numbers))

        self._template_format = "%s%%0%dd%s" % (pfx, self._template.count("#"), sfx)
    ...
    def __getitem__(self, i):
        ...
        if j < self._image_number_range[0] or j > self._image_number_range[1]:
...
class sweep_factory:
    ...
    def sweep(argument, check_headers=False):
        if type(argument) == type([]):
            ...
            return sweep_of_images(find_matching_images(argument), check_headers)
