...
class ImageRecord(object):
    ...
    def __eq__(self, rhs):
        return (
            self.beam == rhs.beam
            and self.detector == rhs.detector
            and self.goniometer == rhs.goniometer
            and self.scan == rhs.scan
            and self.template == rhs.template
...
class DataBlock(object):
    ...
    def extract_stills(self):
        ...
        stills = [(k, r) for k, r in self._images.iteritems() if r.template == None]
        if len(stills) == 0:
            return None
    ...
    def _create_record(self, filename, last=None):
        ...
        return ImageRecord(
            beam=b, detector=d, goniometer=g, scan=s, template=template, index=index
        )
    ...
    def __eq__(self, rhs):
        ...
        return self._format_class == rhs._format_class and self._images == rhs._images
    ...
    def to_dict(self):
        ...
        b, d, g, s = zip(
            *[
                (i.beam, i.detector, i.goniometer, i.scan)
        ...
        for f in filenames:
            try:
                ...
                try:
                    ...
                    continue
                if verbose:
                    print("Starting datablock %d" % len(datablock_list))
            if verbose:
                print("Loaded file: %s" % f)
        ...
        for imageset in d["imageset"]:
            ...
            if imageset["__id__"] == "ImageSweep":
                ...
                if "hierarchy" in dlist[imageset["detector"]]:
                    ...
                    detector = HierarchicalDetector.from_dict(
                        dlist[imageset["detector"]]
                    )
                else:
                ...
                for image in imageset["images"]:
                    ...
                    if image.get("detector", None) is not None:
                        if "hierarchy" in dlist[image["detector"]]:
                            ...
                            record.detector = HierarchicalDetector.from_dict(
                                dlist[image["detector"]]
                            )
                        else:
                            record.detector = Detector.from_dict(
                                dlist[image["detector"]]
                            )
                    if image.get("goniometer", None) is not None:
                        record.goniometer = Goniometer.from_dict(
                            glist[image["goniometer"]]
                        )
...
class DataBlockDumper(object):
    ...
    def as_json(self, filename=None, compact=False):
        ...
        if compact:
            ...
            json.dump(dictionary, open(filename, "w"), indent=2, ensure_ascii=True)
