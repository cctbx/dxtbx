...
class DataBlock(object):
    ...
    def to_dict(self):
        ...
        for iset in self._imagesets:
            if isinstance(iset, ImageSweep):
                if iset.reader().get_format_class() is not None and issubclass(
                    iset.reader().get_format_class(), FormatMultiImage
                ):
                ...
                result["imageset"].append(
                    OrderedDict(
                        [
                            ...
                            (
                                "pedestal",
                                abspath_or_none(iset.external_lookup.pedestal.filename),
                            ),
                ...
                for i in range(len(iset)):
                    ...
                    image_dict["mask"] = abspath_or_none(
                        iset.external_lookup.mask.filename
                    )
