...
int64_t bshuf_compress_lz4_block(ioc_chain *C_ptr, const size_t size,
  ...
  nbytes = LZ4_compress((const char *)tmp_buf_bshuf, (char *)tmp_buf_lz4,
                        size * elem_size);
  free(tmp_buf_bshuf);
...
int64_t bshuf_decompress_lz4_block(ioc_chain *C_ptr, const size_t size,
                                   const size_t elem_size) {
  ...
  nbytes = LZ4_decompress_fast((const char *)in + 4, (char *)tmp_buf,
                               size * elem_size);
...
#else
  nbytes = LZ4_decompress_safe((const char *)in + 4, (char *)tmp_buf,
                               nbytes_from_header, size * elem_size);
  CHECK_ERR_FREE_LZ(nbytes, tmp_buf);
...
size_t bshuf_compress_lz4_bound(const size_t size, const size_t elem_size,
                                size_t block_size) {
