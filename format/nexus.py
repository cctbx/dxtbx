...
class DetectorFactoryFromGroup(object):
    ...
    def __init__(self, instrument, beam, idx=None):
        ...
        for nx_detector in instrument.detectors:
            ...
            assert (
                len(nx_detector.handle["depends_on"]) % n_modules == 0
            ), "Couldn't find NXdetector_modules matching the list of dependencies for this detector"
            ...
            for module_number, nx_detector_module in enumerate(modules):
                ...
                depends_on = nx_detector.handle[
                    nx_detector.handle["depends_on"][(idx // n_modules) + module_number]
                ]
                ...
                slow = nx_detector_module.handle[depends_on.attrs["depends_on"]].attrs[
                    "vector"
                ]
...
class DetectorGroupDataFactory(DataFactory):
    ...
    def __init__(self, obj, instrument):
        ...
        for detector_name in mapping:
            ...
            datalists.append(
                MultiPanelDataList(mapping[detector_name]["datasets"], modules)
            )
